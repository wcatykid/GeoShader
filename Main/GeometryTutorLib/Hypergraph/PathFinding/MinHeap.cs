...
namespace GeometryTutorLib.Hypergraph
{
    ...
    public class MinHeap
    {
        ...
        public MinHeap(int sz)
        {
            ...
            node.key = Double.NegativeInfinity;
            heap[0] = node;
        }
        ...
        //
        public HeapNode<int> ExtractMin()
        {
        ...
        private void Heapify(int index)
        {
            ...
            while (!IsLeaf(index))
            {
                minChild = LeftChildIndex(index);

                // If we're within the bounds of the valid data
                if (minChild < Count)
                {
                    // left child is greater than the right child
                    if (heap[minChild].key > heap[minChild + 1].key)
                    {
                        minChild++;
                    }
                }

                if (heap[index].key <= heap[minChild].key) return;
